---
title: Linux(9)——Python3.6安装
date: 2020-11-17
author: LM
---

## 1.前言

Linux系统自带Python2，这是由于部分命令需要使用Python2，如yum，要使用Python3则需另外安装

```bash
python -V #查看Py版本
python -m XXXX  #执行终端命令，-m参数使得Python预先import你要的package或module给你，然后再执行script。
which python #查看python命令是连接到那个文件
####
/usr/bin/python
####
cd /usr/bin/ #进入bin文件夹
ll python*  #列出包含python字段的文件
####
lrwxrwxrwx. 1 root root   16 11月 17 11:42 python -> /usr/bin/python2
lrwxrwxrwx. 1 root root    9 11月 13 11:01 python2 -> python2.7
-rwxr-xr-x. 1 root root 7144 10月 14 22:46 python2.7
-rwxr-xr-x. 1 root root 1835 10月 14 22:45 python2.7-config
lrwxrwxrwx. 1 root root   16 11月 16 17:40 python2-config -> python2.7-config
lrwxrwxrwx. 1 root root   14 11月 16 17:40 python-config -> python2-config
####
#可以看出python指向python2，最终指向python2.7
```

## 2.安装Python3.6

【注意：高于3.6版本的安装可能不适用于这个教程】

```bash
yum -y install zlib zlib-devel bzip2-devel openssl-devel ncurses-devel sqlite-devel readline-devel tk-devel libffi-devel gcc gcc-c++ make
#安装依赖，用来编译文件
#下载Python3.6源码，解压，然后进入文件夹，编译到对应目录，安装
tar -xvf Python-3.6.2.tar.xz
cd Python-3.6.2
./configure --prefix=/usr/local/python3 --enable-optimizations
# 编译 --prefix=指定文件夹
make && make install
#建立软链接
ln -s /usr/local/python3/bin/python3 /usr/bin/python3
ln -s /usr/local/python3/bin/pip3 /usr/bin/pip3
python3 -V
pip3 -V


# 若要将python换为python3，则先删除原链接再重新指向，这里不推荐这样做-----------------
rm /usr/bin/python
ln -s python3 /usr/bin/python
#如需切换回python2
ln -s python2 /usr/bin/python
# pip
cd /usr/bin
mv pip pip.bak #备份
ln -s pip3 /usr/bin/pip
```

